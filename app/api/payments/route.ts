...
export async function POST(req:Request){try{if(req.headers.get('origin')!==new URL(req.url).origin)return response({error:'Invalid origin'},403);const session=sessionId(req);if(!session)return response({error:'Please reload the shop before checking out.'},401);if(!onlineReady())return response({error:'Online payments are not activated yet.'},503);await limit(req);const b=z.record(z.unknown()).parse(JSON.parse(await boundedText(req)));if(b.action==='create')return response(await createCheckout(b,session,(await getChatGPTUser())?.userId||null));
