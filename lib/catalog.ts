...
export async function catalogResponse(request:Request){
 ...
 const products=(await database().prepare('SELECT id,name,category,description,price,stock,image FROM products WHERE active=1 ORDER BY created DESC LIMIT 200').all()).results;
