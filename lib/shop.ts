...
export async function limit(req:Request,scope='write',maximum=60){const key=req.headers.get('cf-connecting-ip')||'unknown'; const hash=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(key));const id=Array.from(new Uint8Array(hash)).map(x=>x.toString(16).padStart(2,'0')).join('')+':'+scope+':'+Math.floor(Date.now()/60000);const r=await database().prepare('INSERT INTO limits(id,count,expires) VALUES(?,1,?) ON CONFLICT(id) DO UPDATE SET count=count+1 RETURNING count').bind(id,Date.now()+120000).first<any>();if(Math.random()<.02)await database().prepare('DELETE FROM limits WHERE expires < ?').bind(Date.now()).run(); if(r.count>maximum)throw new Error('Too many requests. Please wait a minute.');}
